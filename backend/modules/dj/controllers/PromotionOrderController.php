...
* PromotionOrder
*
* Class PromotionOrderController
* @package backend\modules\dj\controllers
*/
class PromotionOrderController extends BaseController
{
    use Curd;
    ...
    */
    public function actionIndex()
    {
        if (Yii::$app->request->post('hasEditable')) {
            ...
            $model = PromotionOrder::findOne($id);
            $output = '';
            $message = '';
            //由于传递的数据是二维数组，将其转为一维
            $attribute = Yii::$app->request->post('editableAttribute');//获取名称
            $posted = current(Yii::$app->request->post('PromotionOrder'));
            $post = ['PromotionOrder' => $posted];
