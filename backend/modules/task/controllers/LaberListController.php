...
use common\models\task\LaberListTranslations;
use Yii;
 ...
 * LaberList
 *
 * Class LaberListController
 * @package backend\modules\dj\controllers
 */
class LaberListController extends BaseController
{
    use Curd;
     ...
     * @var LaberList
     */
    public $modelClass = LaberList::class;
    ...
    public function actionIndex()
    {
        if (Yii::$app->request->post('hasEditable')) {
            ...
            $model = LaberList::findOne($id);
            $output = '';
            $message = '';
            //由于传递的数据是二维数组，将其转为一维
            $attribute = Yii::$app->request->post('editableAttribute');//获取名称
            $posted = current(Yii::$app->request->post('LaberList'));
            $post = ['LaberList' => $posted];
...
//        $default_lang_model = Languages::find()->select(['code'])->where(['is_default' => 1])->one();
        ...
        $model_translations = LaberListTranslations::find()->where(['lang' => $lang, 'pid' => $id])->one();
        if (empty($model_translations)) {
            $model_translations = new LaberListTranslations();
