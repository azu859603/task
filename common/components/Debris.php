...
class Debris
{
    ...
    protected function getConfigInfo($noCache, $app_id, $merchant_id = '')
    {
        ...
        $cacheKey = 'config:' . $thisAppEnglishName . $merchant_id . $app_id;
    ...
    {
        $config = Config::find()->where(['name' => $name])->with(['value'])->one();
        $config->value->data = is_array($value) ? Json::encode($value) : $value;
        if(!$config->value->save(false)){
            return false;
        }
