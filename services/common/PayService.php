...
//                }
                // 处理充值订单
                $model = RechargeBill::find()->where(['sn' => $log->out_trade_no, 'status' => 3])->one();
                if (empty($model)) {
                    throw new UnprocessableEntityHttpException('订单号错误');
                }
                $model->status = 1;
                $model->save(false);
...
//                Gateway::sendToGroup($thisAppEnglishName . '_admin_message', Json::encode($data));
                break;
        }
    }
