...
/* datatables */
var $table = $('.rf-table');
function buildTable($el, fixedRight) {
    $table.DataTable( {
        scrollX:        false,
        scrollCollapse: true,
        paging:         false,
    ...
    var allThLength = $('.dataTable thead tr').eq(0).find('th').length;
    var fixedCount = allThLength - fixedRight - 1;
    var rightOffset = 0;
    $($('.dataTable thead tr:eq(0) th:gt('+fixedCount+')').toArray().reverse()).each(function () {
        $(this).addClass('dtfc-fixed-right')
        $(this).css({'position' : 'sticky', 'right' : rightOffset + 'px'})
        rightOffset += $(this).outerWidth(true);
    });
}

$(function() {
    var fixedRight = $($table).attr('fixedRight');
    if (fixedRight <= 0) {
        fixedRight = 0;
    }
    if ($($table).find('tbody tr td').eq(1).length !== 0 && fixedRight > 0) {
        buildTable($table, fixedRight);
    }
});
