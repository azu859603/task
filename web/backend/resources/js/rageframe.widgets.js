 ...
 */
function showCheckboxTree(data, id, checkId, autoOpen = false) {
    var that = $("#" + id);
 ...
 * 上下级关系树
 */
function relationsTree(data, id, checkId, autoOpen = false, url, cid) {
    var that = $("#" + id);
    that.jstree({
        "plugins": [
            "contextmenu", "search", "types", "checkbox"
        ],
        "core": {
            'check_callback': true,
            "data": url ? {
                ...
                "success": function(metadata, textStatus, jqXHR) {
                    if (metadata.parent != '#') {
                        ...
                        text += '('+metadata.length+'人)';
                        $("#children").jstree('rename_node', tree_node , text );
                    }
                }
            }: data,
        },
            ...
            "items": {
                ...
                "rename": {
                    ...
                    "action": function (node) {
                        var str = $(node.reference).text();
                        var nstr = str.replace(/\([^\)]*\)/g,"");
                        var a_link = $(node.reference);
                        $(node.reference).after('<input value="'+nstr+'" class="jstree_input">');
                        $('.jstree_input').focus();
                        $('.jstree_input').blur(function () {
                            $(this).after(a_link);
                            $(this).remove();
                        });
                        $(node.reference).remove();
                    }
                },
                "remove": null,
                "ccp": null,
    ...
    // that.on('changed.jstree', function (e, data) {
