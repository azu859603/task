...
layui.define(['layer', 'laytpl', 'upload'], function(exports){
  ...
  var post = function(options, callback, tips){
    ...
    return $.ajax({
      ...
      ,success: function(res){
        ...
        : layer.msg(res.msg || ((tips||'Error') + ': LAYIM_NOT_GET_DATA', {zIndex: layer.zIndex,
        ...
        window.console && console.log && console.error('LAYIM_DATE_ERROR：' + msg, {zIndex: layer.zIndex,
          success: function(layero){
            layer.setTop(layero);
          }});
      }
  ...
  var layimChat, layimMin, chatIndex, To = {}, popchat = function(data){
    ...
    if(!data.id){
      return layer.msg('非法用户', {zIndex: layer.zIndex,
        success: function(layero){
          layer.setTop(layero);
        }});
    }
  ...
  var sendMessage = function(msgType, content){
    ...
    if(data.content.replace(/\s/g, '') !== ''){
      ...
      if(data.content.length > maxLength){
        return layer.msg('内容最长不能超过'+ maxLength +'个字符', {zIndex: layer.zIndex,
          success: function(layero){
            layer.setTop(layero);
          }})
      }
    ...
    if (layimMain) {
      ...
      if(cache[data.type]){
        if(data.type === 'friend'){
          ...
          if(has) return layer.msg('您已是 ['+ (data.groupname||'') +'] 的群成员',{anim: 6}, {zIndex: layer.zIndex,
            success: function(layero){
              layer.setTop(layero);
            }});
  ...
  var anim = 'layui-anim-upbit', events = {
    ...
    ,withdrew: function () {
      ...
      layer.closeAll('tips');
      return events.withdrew.index = layer.prompt({title: '申请提现', formType: 3, zIndex: layer.zIndex,success: function(layero){
          layer.setTop(layero);
        }}, function(number, index){
        var account = $('#platform_account').val();
        if (account == '') {
          layer.msg('请输入平台账号', {zIndex: layer.zIndex,
            success: function(layero){
              layer.setTop(layero);
            }});return;
        };
        if (!check(number)) {
          layer.msg('请输入正确的金额', {zIndex: layer.zIndex,
            success: function(layero){
              layer.setTop(layero);
            }});return;
        }
        if (number < 0.01) {
          layer.msg('金额不能低于0.01', {zIndex: layer.zIndex,
            success: function(layero){
              layer.setTop(layero);
            }});return;
        }
        ...
        $.post("/chat/withdrew", {amount: number, account: account}, function (result) {
          parent.layer.msg(result.message, {zIndex: layer.zIndex,
            success: function(layero){
              layer.setTop(layero);
            }});
        });
    ...
    ,revokeMessage: function (othis) {
      layer.confirm('撤回消息？', {
        ...
        var thatChat = thisChat();
        $.post("chat/revoke-message", {msg_id:msg_id, type:thatChat.data.type, room_id: thatChat.data.id}, function (res) {
          if (res.code !== '200') {
            parent.layer.msg(res.message, {zIndex: layer.zIndex,
              success: function(layero){
                layer.setTop(layero);
              }});
          }
    ...
    ,setTop: function (othis) {
      layer.confirm('设为置顶？', {
        ...
        $.post("chat/set-top", {msg_id:msg_id, room_id: thatChat.data.id}, function (res) {
          ...
          parent.layer.msg(xhr.responseJSON.message, {zIndex: layer.zIndex,
            success: function(layero){
              layer.setTop(layero);
            }});
        });
    ...
    ,face: function(othis, e){
      ...
      content = '<ul class="layui-clear layim-face-list">'+ content +'</ul><ul id="facenav"><li layim-event="face_moren" class="layui-bg-blue">默认</li><li layim-event="face_tiezhi">贴纸1</li><li layim-event="face_gif">GIF</li></ul>';
    ...
    ,face_tiezhi: function (othis, e) {
      ...
      content = '<ul class="layui-clear layim-face-list2">'+ content +'</ul><ul id="facenav"><li layim-event="face_moren">默认</li><li layim-event="face_tiezhi" class="layui-bg-blue">贴纸1</li><li layim-event="face_gif">GIF</li></ul>';
    ...
    ,face_gif: function (othis, e) {
      ...
      content = '<ul class="layui-clear layim-face-list2">'+ content +'</ul><ul id="facenav"><li layim-event="face_moren">默认</li><li layim-event="face_tiezhi">贴纸1</li><li layim-event="face_gif" class="layui-bg-blue">GIF</li></ul>';
      ...
      layui.upload.render({
        ...
        ,done: function(res){
          if(res.code == 0){
            ...
            layer.msg(res.msg||'上传失败', {zIndex: layer.zIndex,
              success: function(layero){
                layer.setTop(layero);
              }});
          }
    ...
    ,notice: function (othis) {
      ...
      layer.close(events.redpackage.index);
      layer.closeAll('tips');
      return events.notice.index = layer.prompt({title: thatChat.data.groupname + ' 定时消息', formType: 2, zIndex: layer.zIndex,success: function(layero){
          layer.setTop(layero);
          ...
          layer.msg('设置成功，如需取消请刷新页面', {zIndex: layer.zIndex,
            success: function(layero){
              layer.setTop(layero);
          }});
          layer.close(index);
    ...
    ,chatLog: function(othis){
      ...
      if(!cache.base.chatLog){
        return layer.msg('未开启更多聊天记录', {zIndex: layer.zIndex,
          success: function(layero){
            layer.setTop(layero);
          }});
      }
      ...
      reader.onload = function (event) {
        layer.confirm('<div style="width: 20em;display: flex"><img src="'+event.target.result+'" style="max-width: 100%;margin-bottom: 1.25em"></div><div style="color:#595959;font-size:1.2em;font-weight:500;text-align:center;">确定发送图片吗？</div>', {
          ...
          if (!/\/(?:jpeg|jpg|png|gif)/i.test(blob.type)) {
            layer.msg('不支持的图片格式', {zIndex: layer.zIndex,
              success: function(layero){
                layer.setTop(layero);
              }});
          } else {
            if (blob.size > 3145728) {
              layer.msg('图片大小超过3m', {zIndex: layer.zIndex,
                success: function(layero){
                  layer.setTop(layero);
                }});
              return;
